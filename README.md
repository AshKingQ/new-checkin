...
**注意事项：**
- 本演示中，数据库的密码字段存储的是 bcrypt 哈希值，而漏洞代码直接比较明文密码
- 这使得正常登录（使用正确的明文密码）会失败，但 SQL 注入攻击（绕过密码检查）仍然有效
- 在真实场景中，如果系统不使用密码哈希，或者攻击者通过其他 SQL 注入手段获取数据，危害会更大
- 本演示的重点是展示如何通过 SQL 注入绕过身份验证逻辑
